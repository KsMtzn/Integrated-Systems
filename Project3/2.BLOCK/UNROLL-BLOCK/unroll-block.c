	...
	for (i=1; i<N+1; i++){
		for (j=1; j<M+1; j+=4){
			...
			for (k=0; k<3; k++){
				...
				for (k=0; k<3; k++){
				block[k+i-1][j]=block[k+i-1][j+1];
				block[k+i-1][j+1]=block[k+i-1][j+2];
				block[k+i-1][j+2]=A[k+i-1][j+3];
			}
				for (k=0; k<3; k++){
				block[k+i-1][j+1]=block[k+i-1][j+2];
				block[k+i-1][j+2]=block[k+i-1][j+3];
				block[k+i-1][j+3]=A[k+i-1][j+4];
			}
				for (k=0; k<3; k++){
				block[k+i-1][j+2]=block[k+i-1][j+3];
				block[k+i-1][j+3]=block[k+i-1][j+4];
				block[k+i-1][j+4]=A[k+i-1][j+5];
			}
	}
